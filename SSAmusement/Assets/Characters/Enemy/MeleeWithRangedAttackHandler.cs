...
public class MeleeWithRangedAttackHandler : MeleeAttackHandler {
    ...
    public bool IsInProjectileAttackInRange() {
        Vector2 distance = target.stats.center_mass.position - projectile_spawn_transform.position;
    ...
    public bool IsTooCloseRanged() {
        float distance =  Mathf.Abs(target.stats.center_mass.position.x - transform.position.x);
